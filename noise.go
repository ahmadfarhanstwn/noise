...
func Fbm2(x, y, frequency, gain, lacunarity float32, octaves int) float32 {
	var sum float32
...
func Turbulence(x, y, frequency, gain, lacunarity float32, octaves int) float32 {
	var sum float32
	amplitude := float32(1)
	for i := 0; i < octaves; i++ {
	 	temp := Snoise2(x*frequency, y*frequency)*amplitude
		if temp < 0 {
			temp *= -1
		}
		sum += temp
		...
		go func (i int) {
			...
			for j := start; j < end; j++ {
				...
				if noiseType == TURBULENCE {
					noise[j] = Turbulence(float32(x), float32(y), freq, gain, lac, octaves)
				} else if noiseType == FBM {
					noise[j] = Fbm2(float32(x), float32(y), freq, gain, lac, octaves)
				}
